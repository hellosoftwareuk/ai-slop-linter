...
const RULE_REMEDIATIONS: &[RuleRemediation] = &[
    ...
    RuleRemediation { rule: "else-after-exit", guidance: "Remove the else wrapper only when the preceding branch always exits and hoisting cannot change lexical scope or comments." },
    RuleRemediation { rule: "terminal-guard-clause", guidance: "Invert a final multi-step branch into an early return only in a non-generator function and only when hoisting cannot change lexical scope." },
    RuleRemediation { rule: "single-use-local-alias", guidance: "Inline a one-read function-local const alias only when semantic bindings prove the source is stable and unshadowed; apply declaration removal and reference replacement atomically." },
    RuleRemediation { rule: "duplicate-branch-body", guidance: "Combine adjacent else-if conditions with short-circuiting OR only when their bodies are AST-identical, comment-free, and contain no identifier reads whose TypeScript narrowing could change." },
